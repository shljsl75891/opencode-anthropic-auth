...
import { retryableAnthropicStreamError } from './stream-error.ts'
...
const MAX_SSE_REWRITE_PENDING_BYTES = 1_048_576
...
export function createServerSideFallbackStreamRewriter() {
  ...
  const drain = () => {
    ...
    if (pending.length > MAX_SSE_REWRITE_PENDING_BYTES) {
      // Forwarding the buffer as a whole event would emit a half-parsed
      // fallback block verbatim, defeating the point of hiding it.
      pending = ''
      throw retryableAnthropicStreamError(
        undefined,
        'SSE event exceeded the maximum buffered size without a boundary',
      )
    }
    return output
  }
